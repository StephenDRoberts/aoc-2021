    ...
    fun checkAdjacents(inputs: List<List<String>>, startingPoints: List<Pair<Int, Int>>, checkedPairs: MutableList<Pair<Int, Int>>): List<Pair<Int, Int>> {
        if (startingPoints.isEmpty()) return checkedPairs else {
            ...
            val newAdjacents = startingPoints.flatMap { startingPoint ->
                ...
                val right =
                    if (column == (inputs[row].size - 1)) mapOf(Pair(10, 10) to 10) else mapOf(
                        Pair(
                            row,
                            minOf(column + 1, inputs[row].size - 1)
                        ) to inputs[row][minOf(column + 1, inputs[row].size)].toInt()
                    )
                val down =
                    if (row == inputs.size - 1) mapOf(Pair(10, 10) to 10) else mapOf(
                        Pair(minOf(row + 1, inputs.size - 1), column) to inputs[minOf(
                            row + 1,
                            inputs.size - 1
                        )][column].toInt()
                    )
